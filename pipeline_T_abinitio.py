...
num_imgs = 25    # Number of images in our source.
noise_variance = 5e-7   # Control noise added to simulated projections


path = fetch_data("emdb_10835.map")

with mrcfile.open(path) as mrc:
    data = mrc.data

og_vol = Volume.load(path, symmetry_group="T")
...
file_name = f'T_projections_{num_imgs}_size_{img_size}_noise_{noise_variance}.mrc'
with mrcfile.new(file_name, overwrite=True) as mrc:
    mrc.set_data(numpy_projections.astype(np.float32))

estimated_volume, est_rots = cryo_abinitio_TO('T', file_name, 'reconstructed_' + file_name,
